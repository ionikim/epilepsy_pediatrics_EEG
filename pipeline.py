...
# Show figures inline when running in VS Code Interactive Window (# %% mode)
if "ipykernel" in sys.modules:
    _orig_close = plt.close
    def _close_and_show(fig=None):
        plt.show()
        _orig_close(fig)
    plt.close = _close_and_show
...
def download_raw_data():
    ...
    GITHUB_RAW = "https://github.com/ionikim/epilepsy_pediatrics_EEG/raw/main/data/raw/"
...
def run_stream_moore():
    ...
    print(f"  Communities: {len(unique):,} | runtime: {runtime:.2f}s | mem: {peak/1e6:.1f} MB")
    ...
    def fmt(v): return "   inf  " if np.isinf(v) else f"{v:8.4f}"
    W = 34
    print(f"\n{'='*67}")
    print(f"  BENCHMARK — Moore Streaming · CHB-01 chb01_03")
    print(f"{'='*67}")
    ...
    for nm, iv, av in [("Intra-Cluster Edge Density", ii, ai),
                       ...
                       ("Conductance", ic, ac),
                       ("Avg Clustering Coeff", icc, acc)]:
        print(f"  {nm:<{W}} {fmt(iv)}  {fmt(av)}")
    print(f"  {'-'*62}")
    print(f"  {'Runtime':<{W}} {runtime:8.2f} s")
    print(f"  {'Peak Memory':<{W}} {peak/1e6:8.1f} MB")
    print(f"{'='*67}")
