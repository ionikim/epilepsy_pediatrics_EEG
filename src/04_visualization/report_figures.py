...
Outputs (reports/figures/):
  fig_hierarchical_heatmap.png    — correlation matrix reordered by Ward cluster
  fig_hierarchical_dendrogram.png — Ward dendrogram colour-coded by cluster
  fig_spectral_raster.png         — cluster assignment per channel over time
  fig_spectral_comparison.png     — interictal vs ictal dominant cluster per channel
"""
...
THRESHOLD    = 0.3     # spectral adjacency threshold
...
# Cluster palette: lilac, baby blue, pastel green, pastel yellow
CLUSTER_COLORS = ["#B39DDB", "#81C3E8", "#88CFA4", "#F5D87A"]
PASTEL_COLORS  = ["#B39DDB", "#81C3E8", "#88CFA4", "#F5D87A"]
ONSET_COLOR    = "#C0392B"
INTER_COLOR    = "#2980B9"
ICTAL_COLOR    = "#E67E22"
...
print("Computing channel temporal profiles …")

def phase_corr(t_start, t_end):
    """Pearson correlation matrix for one phase."""
...
ONSET = int(ONSET_SEC * FS)   # 4096 — seizure onset sample

corr_full  = phase_corr(0,     N_TP)   # full window (for heatmap)
corr_inter = phase_corr(0,     ONSET)  # interictal only
corr_ictal = phase_corr(ONSET, N_TP)   # ictal only

def ward(c):
    """Linkage, flat clusters (0-indexed) and leaf order for a correlation matrix."""
...
# use full-window clustering for the heatmap (existing behaviour)
linkage, flat, order = ward(corr_full)

# phase-specific clustering for the two dendrograms
linkage_inter, flat_inter, order_inter = ward(corr_inter)
linkage_ictal,  flat_ictal,  order_ictal  = ward(corr_ictal)

corr = corr_full   # alias used by the heatmap below

# ── spectral clustering (sliding windows) ─────────────────────────────────
print("Running spectral clustering per window …")
...
all_eigenvalues = np.array(all_eigenvalues)   # (n_windows, N_CH)

all_labels = np.array(all_labels)   # (n_windows, N_CH)
...
aligned = np.array(aligned)   # (n_windows, N_CH)
...
# ════════════════════════════════════════════════════════════════════════════
# FIG 1 — Hierarchical: Correlation Heatmap
# ════════════════════════════════════════════════════════════════════════════
...
for c in range(K):
    ...
    rect = Rectangle(
        ...
        linewidth=2.5, edgecolor=PASTEL_COLORS[c], facecolor="none", zorder=4
    )
    ax.add_patch(rect)
...
legend_handles = [mpatches.Patch(facecolor=PASTEL_COLORS[c],
                                 edgecolor="#aaaaaa", linewidth=0.5, label=f"Cluster {c+1}")
                  for c in range(K)]
ax.legend(handles=legend_handles, loc="upper left",
          bbox_to_anchor=(1.18, 1.0), title="Clusters", title_fontsize=9)

plt.tight_layout()
out = FIG_DIR / "fig_hierarchical_heatmap.png"
...
# ════════════════════════════════════════════════════════════════════════════
# FIG 2a & 2b — Hierarchical: Interictal vs Ictal Dendrogram
# ════════════════════════════════════════════════════════════════════════════
print("Figure 2: dendrograms (interictal + ictal) …")
sch.set_link_color_palette(PASTEL_COLORS)
...
def plot_dendrogram(lnk, title, accent_color, filename):
    ...
    legend_handles.append(mpatches.Patch(facecolor="#CCCCCC", edgecolor="#aaaaaa",
                                         linewidth=0.5, label="Above cut"))
    legend_handles.append(plt.Line2D([0], [0], color="#666666", linestyle="--",
                                     linewidth=1.2, label=f"Cut  (k={K})"))
...
# ════════════════════════════════════════════════════════════════════════════
# FIG 3 — Spectral: Cluster Raster over Time
# ════════════════════════════════════════════════════════════════════════════
print("Figure 3: spectral raster …")

cmap_disc   = ListedColormap(CLUSTER_COLORS[:K])
...
# x ticks at clean 5-second intervals
target_times = [5, 10, 15, 20, 25]
tick_frames  = [min(range(n_windows), key=lambda i: abs(t_centers[i] - t))
                for t in target_times]
ax.set_xticks(tick_frames)
ax.set_xticklabels([f"{t}s" for t in target_times], fontsize=9)

ax.set_yticks(range(N_CH))
ax.set_yticklabels(CHANNEL_NAMES, fontsize=8.5)
ax.set_xlabel("Window centre (s)", fontsize=11)
ax.set_title(
    "Spectral Cluster Assignment per Channel over Time  ·  k=4  ·  CHB-01 chb01_03",
...
legend_elements = [
    mpatches.Patch(facecolor=CLUSTER_COLORS[c], edgecolor="white",
                   label=f"Cluster {c+1}") for c in range(K)
]
legend_elements.append(plt.Line2D([0], [0], color=ONSET_COLOR,
                                  linestyle="--", linewidth=2, label="Seizure onset"))
ax.legend(handles=legend_elements, loc="upper right", framealpha=0.9, fontsize=9)

plt.tight_layout()
out = FIG_DIR / "fig_spectral_raster.png"
fig.savefig(out, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"  → {out.name}")
...
# ════════════════════════════════════════════════════════════════════════════
# FIG 4 — Spectral: Interictal vs Ictal Dominant Cluster
# ════════════════════════════════════════════════════════════════════════════
print("Figure 4: interictal vs ictal comparison …")

# sort channels by interictal cluster for easy visual grouping
...
fig.suptitle(
    "Dominant Spectral Cluster per Channel  ·  Interictal vs Ictal  ·  CHB-01 chb01_03",
...
for ax, majority, title, accent in zip(
    ...
    for ch_i, (ch_name, cluster) in enumerate(zip(ch_sorted, majority)):
        color = CLUSTER_COLORS[cluster]
        ax.barh(ch_i, 1.0, color=color, alpha=0.85, height=0.72,
                edgecolor="white", linewidth=0.5)
        ax.text(1.04, ch_i, f"Cluster {cluster + 1}",
...
# channel labels on left panel only
axes[0].set_yticks(range(N_CH))
axes[0].set_yticklabels(ch_sorted, fontsize=9)

# mark channels that switched cluster
for ch_i in range(N_CH):
    if changed[ch_i]:
        axes[1].annotate(
            "◀ switched", xy=(0, ch_i), xytext=(-0.4, ch_i),
            fontsize=7.5, color="#888888", va="center", ha="right",
        )

# shared legend
legend_handles = [mpatches.Patch(facecolor=CLUSTER_COLORS[c],
                                 edgecolor="white", label=f"Cluster {c+1}")
                  for c in range(K)]
fig.legend(handles=legend_handles, loc="lower center", ncol=4,
           bbox_to_anchor=(0.5, -0.03), framealpha=0.9, fontsize=9)

plt.tight_layout()
out = FIG_DIR / "fig_spectral_comparison.png"
fig.savefig(out, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"  → {out.name}")

# ════════════════════════════════════════════════════════════════════════════
# FIG 5 — Spectral: Eigengap over Time
# ════════════════════════════════════════════════════════════════════════════
print("Figure 5: eigengap over time …")
...
# phase shading
ax.axvspan(t_centers[0],   ONSET_SEC, alpha=0.08, color=INTER_COLOR, zorder=0)
ax.axvspan(ONSET_SEC, t_centers[-1],  alpha=0.08, color=ICTAL_COLOR, zorder=0)
ax.axvline(ONSET_SEC, color=ONSET_COLOR, linewidth=2.0, linestyle="--",
           zorder=4, label="Seizure onset")
...
ax.set_title(
    "Laplacian Eigengap over Time  ·  larger = stronger cluster structure  ·  CHB-01 chb01_03",
    fontweight="bold", pad=10
)
...
plt.tight_layout()
out = FIG_DIR / "fig_spectral_eigengap.png"
fig.savefig(out, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"  → {out.name}")

# ════════════════════════════════════════════════════════════════════════════
# FIG 6 — Global Network Synchrony
# ════════════════════════════════════════════════════════════════════════════
print("Figure 6: global synchrony …")
...
ax.set_title(
    "Global Network Synchrony  ·  CHB-01  ·  chb01_03  ·  2980–3010 s",
    fontweight="bold", pad=10
)
ax.set_xlim(t_centers[0], t_centers[-1])
ax.set_ylim(bottom=0)
...
plt.tight_layout()
out = FIG_DIR / "fig_global_synchrony.png"
fig.savefig(out, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"  → {out.name}")
